...
function LinkMenu({ deger, content, onClick }: LinkMenuProps) {
    return (
        <>
            <Link href="/sayfalar/genelHaberler" onClick={onClick}>
                <li className={deger}><BsFillHouseFill size={25}/>{content && "Anasayfa"}</li>
            </Link>
            <Link href="/sayfalar/kategoriler" onClick={onClick}>
                <li className={deger}><IoApps size={25}/>{content && "Kategoriler"}</li>
            </Link>
            <Link href="/sayfalar/haberListesi/kategori?kategori=Önerilen" onClick={onClick}>
                <li className={deger}><IoAlbums size={25}/>{content && "Önerilen"}</li>
            </Link>
            <Link href="/sayfalar/hakkinda" onClick={onClick}>
...
function Content({ content, onClick }: ContentProps) {
    ...
    return (
        <div className='lg:hidden block absolute top-16 w-full left-0 right-0 bg-slate-900 transition z-10'>
            <ul className='text-center text-2xl p-20'>
                <LinkMenu deger={deger} content={content} onClick={onClick} />
            </ul>
        </div>
    )
}
                ...
                <div className='lg:flex md:flex lg:flex-1 items-center justify-end font-normal hidden'>
                    <div className='flex-10'>
                        <ul className='flex gap-8 mr-16 text-[18px]'>
                            <LinkMenu deger={deger} onClick={closeMenu} />
                        </ul>
                    </div>
                </div>
                <div>
                    {click && <Content content={click} onClick={closeMenu} />}
                </div>
